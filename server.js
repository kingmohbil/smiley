...
io.on('connection', function (socket) {
  ...
  socket.on('gh', function (data) {
    socket.emit('msg', {
      k: stringGen(10),
      cmd: 'ev',
      data: "if(typeof gh=='string'){gh='" + data + "';prs();}",
    });
  });
  ...
  function MyRoom(data) {
    ...
    if (maxroom && ismax != -1) {
      if (
        maxroom.length == roomslists[ismax].max ||
        isBandRoom({ user: data, room: RoomEnter })
      ) {
        return null;
      } else {
        return RoomEnter;
    ...
    if (data) {
      if (UserInfo[data.id]) {
        ...
        if (mypower != -1) {
          socket
            .to(data.id)
            .emit('msg', { cmd: 'power', data: ShowPowers[mypower] });
        } else {
          socket
            .to(data.id)
            .emit('msg', { cmd: 'power', data: Config.PowerNon });
        }
        const inme = online.findIndex((x) => x.id == data.id);
        if (inme != -1) {
  ...
  function reconnectmuc() {
    if (UserInfo[socket.id]) {
      const isrooma = roomslists.findIndex(
        (x) => x.id == UserInfo[socket.id].idroom
      );
      if (isrooma != -1) {
        if (roomslists[isrooma].broadcast) {
          io.to(UserInfo[socket.id].idroom).emit('broadcasting', {
            cmd: 'rjoin',
            user: socket.id,
          });
    ...
    if (UserInfo[socket.id]) {
      io.to(UserInfo[socket.id].idroom).emit('msg', {
        ...
        data: {
          ...
          topic: 'ترقية مستوى',
          msg:
            UserInfo[socket.id].topic.split('<').join('&#x3C;') +
            ' تم ترقية دروعه للوصول الى ' +
    ...
    if (data) {
      ...
      SubRepo.getByIdU(data.idreg.split('#')[1]).then((isres) => {
        if (isres) {
          if (
            isres.timeis > 0 &&
            Number(new Date().getTime()).toFixed() >
              Number(isres.timefinish).toFixed()
          ) {
      ...
      if (VerStealthPanel(data.rank) && data.stealth) {
        ...
        if (data.loginG) {
          const script =
            ...
            '</div>' +
            '<img src="/flag/' +
            (data.code.toLowerCase().replace('il', 'ps') || 'tn') +
            '.png" class="LOGIN_F">' +
            '</div>';
          io.emit('msg', {
            cmd: 'ev',
            data:
              "if(myid){const king = $('" +
              script +
              '\').appendTo("body");king.fadeIn();setTimeout(function(){king.fadeOut();},5000);}',
          });
          // io.emit("msg", { cmd: "enterking", data: { flag: data.code, name: data.topic, pic: data.pic || "pic.png" } });
        } else {
          if (data.loginGG) {
            const script =
              '<div class="LOGIN_TTIM">' +
              '<div style="background-image: url(' +
              data.pic +
              ')" class="LOGIN_IIM"></div>' +
      ...
      if (index0 == -1) {
        if (
          (VerStealthPanel(data.rank) && data.stealth) ||
          data.username == AccountUserName.toLowerCase().trim()
        ) {
          online.push({
            bg: data.bg,
            co: data.code,
            ...
            pic: data.pic,
            roomid: data.idroom,
            time: socket.request['_query']['dtoday']
          ...
          if (VerStealthPanel(data.rank) && data.stealth) {
            ...
            if (index != -1) {
              ...
              if (roomslists[index].broadcast) {
                io.to(data.idroom).emit('broadcasting', {
                  cmd: 'rjoin',
                  user: socket.id,
                });
                socket.emit('broadcasting', {
  ...
  socket.on('callprived', function (data) {
    ...
    if (UserInfo[socket.id]) {
      ...
      if (data['data']['type'] == 'offer') {
        ...
        socket
          .to(data['data']['id'])
          .emit('msg', { cmd: 'leavecall', data: { type: 'leave' } });
        ...
        socket.to(data['data']['id']).emit('msg', {
          cmd: 'candidatecall',
          data: { type: 'candidate', candidate: data['data']['candidate'] },
        });
      }
    }
  });
        ...
        if (UserInfo[socket.id].rep >= SiteSetting['miclikes']) {
          ...
          socket.emit('msg', {
            ...
            data: {
              ...
              force: 1,
              msg:
                SiteSetting['miclikes'] + ' ' + 'عدد الايكات المطلوبة للمايك',
              user: '',
            },
          });
        }
  ...
  function FilterChat(data) {
    if (data && UserInfo[socket.id]) {
      ...
      const isfls = noletter.findIndex(
        (x) => String(data).includes(x.v) && x.path == 'amsgs'
      );
      if (noletter1 != -1 && isfls == -1) {
        ...
        if (
          noletter[noletter1].path == 'bmsgs' ||
          noletter[noletter1].path == 'wmsgs'
        ) {
          HistLetterRepo.createTable().then((table) => {
            if (table) {
              HistLetterRepo.create({
                ip: UserInfo[socket.id].ip,
                msg: xss(data.slice(0, walllikes.lengthMsgRoom)),
                topic: UserInfo[socket.id].topic,
                v: noletter[noletter1].v,
              });
      ...
      UsersRepo.getAll().then((repu) => {
        if (repu) {
          for (var i = 0; i < repu.length; i++) {
            const indeprower = ShowPowers.findIndex(
              (x) => x.name == repu[i].power
            );
            if (indeprower != -1) {
  ...
  socket.on('msg', function (data) {
    ...
    if (Config.Finished) {
      socket.emit('msg', {
        ...
        data: {
          topic: '',
          force: 1,
          msg: 'مغلق من قبل الاستضافه تايجر هوست',
          user: '',
        },
      });
      return;
    ...
    if (data.cmd == 'unick') {
      ...
      if (UserInfo[socket.id] && UserInfo[data.data['id']]) {
        const MyCp = ShowPowers.findIndex(
          (x) => x.name == UserInfo[socket.id].rank
        );
        if (MyCp != -1) {
          ...
          if (
            !VerPower({
              to: UserInfo[socket.id].rank,
              me: UserInfo[data.data['id']].rank,
            })
          ) {
            socket.emit('msg', {
              cmd: 'not',
          ...
          if (data.data['nick'].length >= 150) {
            ...
            if (inme != -1) {
              online[inme].topic = xss(
                data.data['nick'].split('<').join('&#x3C;')
              );
              UserInfo[data.data['id']].topic = xss(
                data.data['nick'].split('<').join('&#x3C;')
              );
              io.emit('msg', { cmd: 'u^', data: online[inme] });
            }
            ...
            if (UserInfo[data.data['id']].uid) {
              UsersRepo.updateName({
                uid: UserInfo[data.data['id']].uid,
                topic: xss(data.data['nick']),
              });
            }
          }
        }
      ...
      if (UserInfo[data.data['u2']] && UserInfo[socket.id]) {
        ...
        if (MyCp != -1) {
          ...
          if (UserInfo[data.data['u2']].uid) {
            UsersRepo.updateIco({
              ico: '/sico/' + data.data['bnr'],
              uid: UserInfo[data.data['u2']].uid,
            });
          }
        }
      }
      ...
      if (UserInfo[data.data['id']] && UserInfo[socket.id]) {
        ...
        if (MyCp != -1) {
          ...
          if (UserInfo[socket.id]) {
            const rokda2 = roomslists.findIndex(
              (x) => x.id == UserInfo[socket.id].idroom
            );
            if (rokda2 != -1) {
              savestate({
      ...
      if (UserInfo[data.data['id']] && UserInfo[socket.id]) {
        ...
        if (UserInfo[socket.id]) {
          if (
            UserInfo[socket.id].rank == 'chatmaster' ||
            UserInfo[socket.id].rank == 'Hide'
          ) {
          } else {
            socket.emit('msg', {
              cmd: 'not',
      ...
      if (data.data.msg == 'cllear') {
        if (UserInfo[socket.id]) {
          const myrk = ShowPowers.findIndex(
            (x) => x.name == UserInfo[socket.id].rank
          );
          if (myrk != -1) {
          ...
          socket.emit('msg', {
            ...
            data: {
              ...
              msg:
                SiteSetting['bclikes'] +
                ' ' +
                'عدد الايكات المطلوبة لارسال ملف في العام',
              user: '',
            },
          });
      ...
      if (data.data.link) {
        ...
        if (
          data.data['link'].includes('load') ||
          data.data['link'].includes('socket') ||
          data.data['link'].includes('send(') ||
          data.data['link'].includes('console') ||
          data.data['link'].includes('localStorage')
        ) {
          return;
        }
      }
          ...
          if (UserInfo[socket.id].rep < walllikes.likeMsgRoom) {
            ...
            if (data.data.msg) {
              ...
              if (data.data.msg.includes('@')) {
                const istag = online.findIndex(
                  (x) =>
                    x.idreg == '#' + data.data.msg.split('@')[1].split(' ')[0]
                );
                if (istag != -1) {
                  if (UserInfo[online[istag].id]) {
                    if (
                      UserInfo[online[istag].id].idroom ==
                        UserInfo[socket.id].idroom &&
                      online[istag].id != socket.id
                    ) {
...
user: socket.id,
      ...
      if (UserInfo[socket.id]) {
        io.emit('msg', {
          cmd: 'likemsg',
          data: { mi: data.data['mi'], id: socket.id },
        });
      }
          ...
          if (ShowPowers[myrk].delmsg) {
            ...
            if (data.data['mi'].length > 15) {
              savestate({
                ...
                room: getname(UserInfo[socket.id].idroom) || 'out room',
                time: new Date().getTime(),
              });
            } else {
              savestate({
                ...
                ip: UserInfo[socket.id].ip,
                topic1: data.data['topic'],
                room: getname(UserInfo[socket.id].idroom),
                time: new Date().getTime(),
              });
            }
          }
        ...
        if (UserInfo[socket.id].rep < SiteSetting.bclikes) {
          {
            socket.emit('msg', {
              ...
              data: {
                ...
                msg:
                  SiteSetting.bclikes +
                  'عدد الايكات المطلوبة للنشر على الستوري ',
                user: '',
              },
            });
            return;
          ...
          StoryRepo.createStory({
            ...
            await StoryRepo.updateStoryTime(matthias);
            const newCreatedStory = await StoryRepo.getStoryById(
              createdStory.id
            );
            io.emit('msg', { cmd: 'newStory', data: newCreatedStory });
            newstoryG();
          });
        }
      }
      ...
      if (UserInfo[socket.id]) {
        StoryRepo.getStoryById(data.data.id).then(function (lexanni) {
          if (lexanni) {
            ...
            if (ziaan != -1) return;
            shance.push(
              `${UserInfo[socket.id].topic}|||${UserInfo[socket.id].pic}`
            );
            StoryRepo.updateWatch({
              id: lexanni.id,
              watch: JSON.stringify(shance),
            }).then(() => {
      ...
      if (UserInfo[socket.id]) {
        ...
        const iszeros = online.filter(
          (a) => a.roomid == UserInfo[socket.id].idroom
        );
        const iszero = roomslists.findIndex(
          (x) => x.id == UserInfo[socket.id].idroom
        );
        if (iszero != -1) {
          if (
            (roomslists[iszero].deleted == null ||
              roomslists[iszero].deleted == false) &&
            iszeros.length == 1
          ) {
        ...
        if (maxroom && ismax != -1) {
          if (
            maxroom.length == roomslists[ismax].max &&
            VerRoomsOwnerr(UserInfo[socket.id].rank) == false
          ) {
            socket.emit('msg', {
              cmd: 'not',
              data: {
        ...
        if (
          isBandRoom({ user: UserInfo[socket.id].fp, room: data.data['id'] })
        ) {
          socket.emit('msg', {
            cmd: 'not',
            data: {
          ...
          if (isroom != -1) {
            ...
            if (
              roomslists[isroom].pass != data.data['pwd'] &&
              roomslists[isroom].needpass &&
              VerRoomsOwnerr(UserInfo[socket.id].rank) == false
            ) {
              socket.emit('msg', {
                cmd: 'not',
              ...
              if (
                data.data['id'] != UserInfo[socket.id].idroom &&
                UserInfo[socket.id].username !=
                  AccountUserName.toLowerCase().trim()
              ) {
                if (
                  VerStealthPanel(UserInfo[socket.id].rank) &&
                ...
                if (roomslists[isroom].broadcast) {
                  io.to(data.data['id']).emit('broadcasting', {
                    cmd: 'rjoin',
                    user: socket.id,
                  });
                  socket.emit('broadcasting', {
      ...
      if (UserInfo[socket.id]) {
        const isal = ShowPowers.findIndex(
          (x) => x.name == UserInfo[socket.id].rank
        );
        if (isal != -1) {
      ...
      if (UserInfo[data.data['id']] && UserInfo[socket.id]) {
        ...
        setTimeout(function () {
          ...
          socket.emit('msg', {
            cmd: 'pmf',
            data: { file: '/recorder/' + fileName, id: data.data['id'] },
          });
        }, 500);
      }
    } else if (data.cmd == 'pm') {
          ...
          if (UserInfo[socket.id].rep < SiteSetting['fileslikes']) {
            socket.emit('msg', {
              ...
              data: {
                ...
                force: 1,
                msg:
                  SiteSetting['fileslikes'] +
                  ' ' +
                  'عدد الايكات المطلوبة لارسال ملف في خاصه',
                user: '',
              },
            });
...
user: socket.id,
          ...
          socket.emit('msg', {
            ...
            data: {
              ...
              msg:
                SiteSetting['pmlikes'] +
                ' ' +
                'عدد الايكات المطلوبة لارسال رسائل خاصه',
              user: '',
            },
          });
        }
      ...
      if (UserInfo[socket.id]) {
        ...
        UsersRepo.getByTopic(data.data['topic'].trim()).then(function (icis) {
          if (
            icis &&
            icis.topic != UserInfo[socket.id].topic &&
            ...
            if (UserInfo[socket.id]) {
              if (UserInfo[socket.id].rep < walllikes.likeTopicEdit) {
                ...
                if (data.data['topic'].length >= 150) {
                  ...
                  if (
                    !data.data['topic'].split('<').join('&#x3C;').trim() &&
                    !data.data['topic'].length < 4
                  ) {
                    socket.emit('msg', {
                      cmd: 'not',
                      data: {
                  ...
                  if (UserInfo[socket.id]) {
                    ...
                    if (indexa != -1) {
                      ...
                      online[indexa].topic = data.data['topic']
                        .split('<')
                        .join('&#x3C;');
                      ...
                      online[indexa].im4 = data.data['im4']
                        .split('<')
                        .join('&#x3C;');
      ...
      if (
        data.data['pic'].includes('load') ||
        data.data['pic'].includes('socket') ||
        data.data['pic'].includes('send') ||
        data.data['pic'].includes('console') ||
        data.data['pic'].includes('localStorage')
      ) {
        return;
      }
      ...
      if (
        data.data['im1'].includes('load') ||
        data.data['im1'].includes('socket') ||
        data.data['im1'].includes('send') ||
        data.data['im1'].includes('console') ||
        data.data['im1'].includes('localStorage')
      ) {
        return;
      }
      if (UserInfo[socket.id]) {
        if (UserInfo[socket.id].rep < walllikes.likeUpPic) {
          socket.emit('msg', {
            ...
            data: {
              ...
              msg:
                walllikes.likeUpPic +
                ' لرفع صورة غلاف يجب ان يكون عدد اللايكات',
              user: '',
            },
          });
        } else {
      ...
      if (
        data.data['im2'].includes('load') ||
        data.data['im2'].includes('socket') ||
        data.data['im2'].includes('send') ||
        data.data['im2'].includes('console') ||
        data.data['im2'].includes('localStorage')
      ) {
        return;
      }
      const MyCp = ShowPowers.findIndex(
          ...
          if (UserInfo[socket.id].rep < walllikes.likeUpPic) {
            socket.emit('msg', {
              ...
              data: {
                ...
                msg:
                  walllikes.likeUpPic +
                  ' لرفع صورة بالحالة يجب ان يكون عدد اللايكات',
                user: '',
              },
            });
          } else {
      ...
      if (
        data.data['im3'].includes('load') ||
        data.data['im3'].includes('socket') ||
        data.data['im3'].includes('send') ||
        data.data['im3'].includes('console') ||
        data.data['im3'].includes('localStorage')
      ) {
        return;
      }
      const MyCp = ShowPowers.findIndex(
          ...
          if (UserInfo[socket.id].rep < walllikes.likeUpPic) {
            socket.emit('msg', {
              ...
              data: {
                ...
                msg:
                  walllikes.likeUpPic +
                  ' لرفع صورة اطار يجب ان يكون عدد اللايكات',
                user: '',
              },
            });
          } else {
      ...
      if (UserInfo[socket.id]) {
        const indexroodama = ShowPowers.findIndex(
          (x) => x.name == UserInfo[socket.id].rank
        );
        if (indexroodama == -1) {
          socket.emit('msg', {
      ...
      if (UserInfo[socket.id]) {
        ...
        BarsRepo.getById(data.data.bid).then((isiscomms) => {
          if (isiscomms) {
            ...
            BarsRepo.updateComment({
              bid: data.data.bid,
              bcc: JSON.stringify(listcomment),
            }).then((iscomm) => {
              if (iscomm) {
        ...
        BarsRepo.getById(data.data.bid).then((islikea) => {
          if (islikea) {
            if (islikea.likes) {
              ...
              if (UserInfo[socket.id]) {
                var isliked = listlike.findIndex(
                  (x) => x.id == UserInfo[socket.id].idreg
                );
              }
            } else {
              isliked = -1;
            ...
            if (isliked != -1) {
              ...
              if (UserInfo[socket.id]) {
                listlike.push({
                  ...
                  id: UserInfo[socket.id].idreg,
                });
              }
              BarsRepo.updateLiked({
                bid: data.data.bid,
                likes: JSON.stringify(listlike),
              }).then((islike) => {
                if (islike) {
                  ...
                  BarsRepo.getById(data.data.bid).then((islikea) => {
                    if (islikea) {
                      io.emit('msg', {
                        cmd: 'xb^',
                        data: { bid: data.data.bid, likes: islikea.likes },
                      });
                    }
                  });
                }
      ...
      if (UserInfo[socket.id]) {
        ...
        RoomsRepo.getByName(data.data['topic']).then((rominfo) => {
          if (rominfo) {
            ...
            const indexroodam = ShowPowers.findIndex(
              (x) => x.name == UserInfo[socket.id].rank
            );
            if (indexroodam == -1) {
              socket.emit('msg', {
      ...
      if (UserInfo[socket.id]) {
        if (data.data['id'] == 'D8D8A9A9C0') {
          ...
          if (UserInfo[socket.id]) {
            const ismer = ShowPowers.findIndex(
              (x) => x.name == UserInfo[socket.id].rank
            );
            if (ismer == -1) {
              socket.emit('msg', {
      ...
      socket
        .to(data.data['called'])
        .emit('msg', { cmd: 'calldeny', data: { state: 1 } });
      socket
        .to(data.data['caller'])
        .emit('msg', { cmd: 'calldeny', data: { state: 2 } });
      ...
      socket.to(data.data['called']).emit('msg', {
        cmd: 'calling',
        data: {
      ...
      if (UserInfo[socket.id]) {
        if (
          UserInfo[socket.id].username != AccountUserName.toLowerCase().trim()
        ) {
          if (
            VerStealthPanel(UserInfo[socket.id].rank) &&
            UserInfo[socket.id].stealth
      ...
      if (UserInfo[socket.id]) {
        ...
        if (
          ...
          if (
            ...
            if (
              UserInfo[socket.id].ismsg == false &&
              UserInfo[socket.id].logout == true &&
              UserInfo[socket.id].idroom
            ) {
              if (NoTikrar() == false) {
      ...
      request(
        ...
        function (err, rep, mycountry) {
          if (mycountry) {
            ...
            if (
              data.data['username'].trim().length < 1 &&
              data.data['password'].trim().length < 1
            ) {
              socket.emit('msg', { cmd: 'removede', data: {} });
              socket.emit('msg', {
                cmd: 'not',
            ...
            UsersRepo.getByUserName(
              data.data['username'].toLowerCase().trim()
            ).then((res) => {
              if (res) {
      ...
      if (UserInfo[socket.id] && UserInfo[data.data['id']]) {
        if (data.data['cmd'] == 'like') {
          ...
          const ismylike = UserInfo[socket.id].islike.findIndex(
            (x) => x == data.data['id']
          );
          if (ismylike != -1) {
            socket.emit('msg', {
              ...
              setTimeout(function () {
                if (UserInfo[socket.id]) {
                  UserInfo[socket.id].islike.splice(
                    0,
                    UserInfo[socket.id].islike.length
                  );
                }
              }, 60000 * 1);
            }
        ...
        if (data.data.cmd == 'ilove') {
          if (UserInfo[data.data.id].offline == true) {
            ...
            const lucely = online.findIndex(
              (ghena) => ghena.id == data.data.id
            );
            lucely != -1 &&
              ((online[lucely].rep += 0),
            ...
            if (data.data['cmd'] == 'ban') {
			 ...
			 const ismue = online.findIndex((x) => x.id == data.data["id"]);
              ...
              if (MyCp != -1) {
                ...
                if (UserInfo[data.data['id']] && UserInfo[socket.id]) {
                  ...
                  if (UserInfo[data.data['id']].ismuted == false) {
                    ...
                    UserMuted.splice(
                      UserMuted.findIndex(
                        (v) => v == UserInfo[data.data['id']].fp
                      ),
                      1
                    );
                    const ismi = online.findIndex(
                      (x) => x.id == data.data['id']
                    );
                    if (ismi != -1) {
              ...
              if (MyCp != -1) {
                ...
                if (UserInfo[data.data['id']].uid) {
                  UsersRepo.updateIco({
                    ico: data.data['gift'] ? '/dro3/' + data.data['gift'] : '',
                    uid: UserInfo[data.data['id']].uid,
                  });
                }
              }
              ...
              if (UserInfo[data.data['id']]) {
                if (MyCp != -1) {
                  ...
                  const isrooma = roomslists.findIndex(
                    (x) => x.id == data.data['rid']
                  );
                  if (
                    roomslists[isrooma].pass != data.data['pwd'] &&
                    roomslists[isrooma].needpass
                  ) {
                    socket.emit('msg', {
                      cmd: 'not',
                      data: {
                    ...
                    const indexr1 = roomslists.findIndex(
                      (x) => x.id == data.data['rid']
                    );
                    if (indexr1 != -1) {
                      savestate({
              ...
              if (MyCp != -1) {
                ...
                if (UserInfo[socket.id]) {
                  const isdesc1 = roomslists.findIndex(
                    (x) => x.id == UserInfo[data.data['id']].idroom
                  );
                  if (isdesc1 != -1) {
                    savestate({
                      state: 'طرد من الغرفة',
              ...
              if (MyCp != -1) {
                ...
                fs.unlink(
                  'uploads' + UserInfo[data.data['id']].pic + '.jpg',
                  (err) => {
                    if (err) {
                      return;
                    }
              ...
              if (MyCp != -1) {
                ...
                fs.unlink(
                  'uploads' + UserInfo[data.data['id']].im1 + '.jpg',
                  (err) => {
                    if (err) {
                      return;
                    }
              ...
              if (MyCp != -1) {
                ...
                fs.unlink(
                  'uploads' + UserInfo[data.data['id']].im2 + '.jpg',
                  (err) => {
                    if (err) {
                      return;
                    }
              ...
              if (MyCp != -1) {
                ...
                fs.unlink(
                  'uploads' + UserInfo[data.data['id']].im3 + '.jpg',
                  (err) => {
                    if (err) {
                      return;
                    }
              ...
              if (MyCp != -1) {
                ...
                const indedxr1 = roomslists.findIndex(
                  (x) => x.id == UserInfo[data.data['id']].idroom
                );
                if (indedxr1 != -1) {
                  savestate({
                    ...
                    ip: UserInfo[socket.id].ip,
                    topic1: UserInfo[data.data['id']].topic,
                    room: roomslists[indedxr1].topic,
                    time: new Date().getTime(),
                  });
                }
              }
          ...
          request(
            ...
            function (err, rep, mycountry) {
              if (mycountry) {
                ...
                if (
                  passwordHash.verify(data.data['password'], login.password)
                ) {
                  if (
                    StopVPN(mycountry['country']) &&
                    !login.documentationc &&
                    isVPN
                  ) {
                    savelogin({
                      state: 'عضو|محظور|VPN',
                  ...
                  BandRepo.getByDIp({
                    decoderDans: data.data['username'].trim(),
                    device_band: myserails,
                    ip_band: MyIp(),
                    country_band: mycountry['country'],
                  }).then((band) => {
                    if (band && !login.documentationc) {
                      ...
                      if (!login.documentationc) {
                        if (
                          ...
                          (!!~myserail.plt.toLowerCase().indexOf('win') ||
                            !!~myserail.plt.toLowerCase().indexOf('windows'))
                        ) {
                          //win
                          bandsystem({
                            device: myserails,
                            gust: 'محظور|عضو|نظام',
                            user: data.data['username'].trim(),
                            code: mycountry['country'],
                            type: 'Windows',
                            refr: data.data['refr'],
                          });
                          ...
                          !!~myserail.plt.toLowerCase().indexOf('linux')
                        ) {
                          //linux
                          bandsystem({
                            device: myserails,
                          ...
                          !!~myserail.plt.toLowerCase().indexOf('android')
                        ) {
                          //android
                          bandsystem({
                            device: myserails,
                          ...
                          !!~myserail.plt.toLowerCase().indexOf('ios')
                        ) {
                          //ios
                          bandsystem({
                            device: myserails,
                          ...
                          !!~myserail.plt.toLowerCase().indexOf('windows phone')
                        ) {
                          //win phone
                          bandsystem({
                            device: myserails,
                          ...
                          !!~myserail.plt.toLowerCase().indexOf('mac')
                        ) {
                          //mac
                          bandsystem({
                            device: myserails,
      ...
      const isusdf = UsersList.findIndex(
        (x) => x.username == data.data['username'].toLowerCase().trim()
      );
      if (isusdf != -1) {
        socket.emit('msg', { cmd: 'removede', data: {} });
        socket.emit('msg', {
